...
def criar_usuario(usuario, senha, nome_completo, email, role='user'):
    ...
    try:
        result = usuarios.insert_one(novo_usuario)
...
def inicializar_usuarios():
    ...
    """
    admin_existente = usuarios.find_one({'usuario': 'admin'})

    if not admin_existente:
        print('📝 Criando usuário admin padrão...')
        resultado = criar_usuario(
            usuario='admin',
            senha='admin',  # ALTERE ESSA SENHA DEPOIS!
...
if __name__ == '__main__':
    ...
    resultado = autenticar_usuario('admin', 'admin')
